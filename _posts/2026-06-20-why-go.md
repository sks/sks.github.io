...
We centralized repair in [`github.com/kaptinlin/jsonrepair`](https://github.com/kaptinlin/jsonrepair) — a high-performance Go library ported from the widely used JavaScript `jsonrepair` project. Our shared decode helpers try strict `encoding/json` first; when that fails, they call `jsonrepair.Repair()` and unmarshal the result:
...
Python's [`json-repair`](https://pypi.org/project/json-repair/) library solves a similar problem on the other side of the fence. In Go, `kaptinlin/jsonrepair` closes most of the gap for *syntax* repair; you still choose your own types at the end.
